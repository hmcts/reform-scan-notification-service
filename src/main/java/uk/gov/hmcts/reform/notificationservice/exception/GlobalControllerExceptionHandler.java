...
public class GlobalControllerExceptionHandler {
     ...
     * Puts the notification info associated with the exception into the response body and returns a status code of 424 (FAILED DEPENDENCY)
     * @param ex the exception
     * @return the response entity
     ...
     * When the body of the request has correct syntax but the values fail validation the MethodArgumentNotValidException
     * is thrown. This method takes the fields that failed validation and the reasons why and puts them in the response body
     ...
     * Puts the malformed number and a message about the number being invalid into the response body with a status code of
     * 400 (BAD REQUEST)
     * @param ex the exception
     * @return the response entity
     * @throws JsonProcessingException
     ...
     * When the authentication library encounters a non-Feign exception trying to authenticate a token the ServiceException is thrown
     * This method handles the exception by returning a status code of 500 (INTERNAL_SERVER_ERROR)
     * @param ex the exception
     * @return the response entity
     */
